...
一个可以部署在 GitHub Pages 的个人 A 股收息观察工具。它把自动获取的已实施分红（失败时使用人工兜底值）和最近有效股价放在一起，计算当前股息率，并为每只股票生成连续的股息率价格网格。
...
- 展示关注股票的现价、自动汇总的已实施年度现金分红和当前股息率；可把某只股票设为人工分红兜底。
- 按“深入击球区 / 已进入 / 接近 / 等待”排序观察；这是估值状态，**不是买卖建议**。
- 点击任意股票，查看 0.1% 步长的股息率对应价格网格。
- 每个工作日北京时间 09:00–15:55 间约每 5 分钟刷新行情；GitHub 排队时可能稍晚。
- 每个工作日北京时间约 19:15 自动检查一次分红明细，只采用“已实施”记录并按报告年度汇总。
- 网页内可添加、删除股票并导出配置；本地修改保存在当前浏览器，不需要登录。
...
点击首页的 **管理本地股票**，即可在当前浏览器添加或删除股票。修改会立即反映在网页上。点击 **导出配置** 会下载 `stocks.json`：将它替换仓库的 `data/stocks.json` 并提交后，GitHub Actions 才会自动获取新股票的行情和分红。这个设计避免在网页中保存 GitHub 密钥。
...
| `annual_dividend` | 每股年度分红的人工兜底值。自动分红失败或将 `dividend_mode` 设为 `manual` 时使用。|
| `dividend_mode` | 可选：`auto`（默认，采用自动分红）或 `manual`（始终采用 `annual_dividend`）。|
| `dividend_note` | 人工分红的来源或维护说明。|
...
流程会读取股票池、抓取并校验价格、写入 `data/market.json`，再自动提交和发布页面。工作日盘中会约每 5 分钟自动运行；GitHub 的定时任务可能因排队而稍晚执行。

分红更新可在 `Actions` → `Update Annual Dividends` → `Run workflow` 手动运行；它每天也会自动检查。
...
└── .github/workflows/
    ...
    ├── update-dividends.yml           # 定时/手动刷新分红
